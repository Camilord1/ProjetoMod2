...
class Pesquisa(Pessoa): # Recebe como herança a classe Pessoa, pois a Pesquisa depende da existência da Pessoa
    ...
    def resgatarDadosPesquisa(self):
        return f'O nome da pessoa que participou da pesquisa é {self.getNome()}, idade {self.idade}, gênero {sexo[self.genero]} e as repostas {self.perg01}, {self.perg02}, {self.perg03} e {self.perg04}'

""" Variáveis globais
"""
condicao = True # Variavel usada no laço para sair do while quando digitar 00
dataHora_atual = datetime.now().strftime('%Y-%m-%d %H:%M') # Variável que armazena a data e hora atual formatada
